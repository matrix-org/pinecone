...
type PublicKey [eddilithium2.PublicKeySize]byte
type PrivateKey [eddilithium2.PrivateKeySize]byte
type Signature [eddilithium2.SignatureSize]byte

func (a PrivateKey) Public() PublicKey {
	var public PublicKey
	ed := eddilithium2.PrivateKey{}
	ed.UnmarshalBinary(a[:])
	edPublic := ed.Public().(*eddilithium2.PublicKey)
	copy(public[:], edPublic.Bytes())
	return public
}
